...
// Table
import TableRow from '../../components/table/TableRowMaterial'
...
// Modals
import { ModalCreateMaterial } from '../../components/modal/material/ModalCreate'
import { ModalEditMaterial } from '../../components/modal/material/ModalEdit'
...
export function Material() {
  // State
  const [rowsData, setRowsData] = useState<MaterialInterface[] | null>(null)
  ...
  // Consts
  const namePageUppercase = 'Materiais'
  const namePageLowercase = 'materiais'
  const namePageSingular = 'material'

  // List Array
  const itemsBreadcrumbs = [
    { label: 'Inicio', to: routsNameMain.home },
    { label: namePageUppercase, to: routsNameMain.material },
    { label: 'Listagem' }
  ]
  ...
  async function fetchData(limit: string) {
    ...
    // Get
    await MaterialViewModel.getAll().then(response => {
      if (response.error) {
        showToast('error', response.msg as string)
      } else {
        const arrayData = response.data as unknown as MaterialInterface[]
        setTotalDocs(arrayData.length)
        console.log(arrayData)

        const listData = arrayData.slice(0, Number(limit))

        setRowsData(listData as MaterialInterface[])
      }
    })
  }
  ...
  async function searchDocs() {
    if (termForSearch == '') {
      ...
      MaterialViewModel.getAllByTermData(termForSearch).then(response => {
        setRowsData(response?.data as MaterialInterface[])
      })
    }
  }
  ...
  function handleDeleteRow(id: string) {
    swal({
      ...
      if (willDelete) {
        await MaterialViewModel.delete(id).then(response => {
