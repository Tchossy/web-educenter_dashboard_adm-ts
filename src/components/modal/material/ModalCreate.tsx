...
export function ModalCreateMaterial({
  ...
  // Pdf
  const [filePdfSelect, setFilePdfSelect] = useState<string>('')
  ...
  const onPdfChange = (e: any) => {
    ...
    setFilePdfSelect(pdfFile)
  }
  const onVideoLinkChange = (e: ChangeEvent<HTMLSelectElement>) => {
  ...
  async function handleUploadPdf(): Promise<{
    urlMateriaPdf: string
  }> {
    // PdfMateria
    const pdfMateriaData = new FormData()
    pdfMateriaData.append('pdfMateria', filePdfSelect)
                ...
                <div className="w-full grid gap-6 md:grid-cols-1">
                  <div className="h-full">
                    ...
                    <input
                      id="link_video"
                      type="text"
                      placeholder="Ex.: https://www.youtube.com/@EduCenter/video1"
                      onChange={e => {
                        onVideoLinkChange(e as any)
                      }}
                      className="w-full bg-gray-700/60 border border-gray-500/60 text-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block"
                    />
                  </div>
